...
def run_qkd_benchmark(key_length):
    ...
    print(f"  - Starting BB84 key generation ({key_length} bits)...")
...
def run_pqc_benchmark():
    ...
    for _ in range(N_ITERATIONS):
        ...
        # We time the signing operation, but discard the variable assignment 
        # to prevent the 'unbound' warning and keep the benchmark clean.
    ...
    for _ in range(N_ITERATIONS):
        ...
        # 'signature' is now guaranteed to be bound to a valid value from the fix above.
...
def run_aes_benchmark():
    ...
    print("  - Starting AES-256-GCM Benchmark...")
...
def main():
    ...
    print("=" * 60)
    print(f"Note: QKD times are highly dependent on the QuNetSim library and local machine speed.")
    print(f"PQC times averaged over {N_ITERATIONS} iterations.\n")

    # 1. PQC Overhead
    print("---- 1. PQC (ML-DSA-65) Overhead ----")
    pqc_results = run_pqc_benchmark()
    print(f"  > Keypair Generation: {pqc_results['keypair_time']:.6f} seconds")
    print(f"  > Avg. Signature Time:  {pqc_results['avg_sign_time'] * 1e6:.3f} microseconds")
    print(f"  > Avg. Verification Time: {pqc_results['avg_verify_time'] * 1e6:.3f} microseconds\n")
    ...
    print(f"  > Avg. Encryption Time: {aes_results['avg_encrypt_time'] * 1e6:.3f} microseconds")
    print(f"  > Avg. Decryption Time: {aes_results['avg_decrypt_time'] * 1e6:.3f} microseconds\n")
    ...
    print(f"  ✓ 128-bit Key Time: {time_128:.2f} seconds")
    ...
    if time_128 < 300: # Max 5 minutes for 128-bit, otherwise skip 256-bit
        ...
        print(f"  ✓ 256-bit Key Time: {time_256:.2f} seconds\n")
    else:
